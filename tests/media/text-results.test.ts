...
import { requireMediaJob, updateMediaJob, mediaDraft } from '../../server/src/media/mediaJobStore.ts';
...
databaseCase('text results use guarded, idempotent chat acceptance and survive finishing', async () => {
  const workflow = {
    id: 'text',
    name: 'Text',
  ...
  const branch = () => {
    const row = stmt('SELECT active_leaf_id, mutation_revision FROM conversations WHERE id = ?').get(conversationId)!;
    return {
  ...
  assert.throws(() => accept({ expectedMutationRevision: Number(originalBranch.expectedMutationRevision) + 1 }), {
    status: 409,
  });
