...
test('endpoint saves preserve API-key removal requested while a submission is pending', async () => {
  ...
  try {
    ...
    assert.equal(
      Object.hasOwn(submitted!, 'apiKey'),
      false,
      'Blank credentials preserve the saved key',
    );
    removeKey();
    ...
    assert.equal(
      editor.data().apiKey,
      '',
      'An earlier save must not acknowledge the later removal',
    );
    ...
    assert.equal(
      editor.data().apiKey,
      'replacement',
      'Typing during removal remains a pending edit',
    );
    fields.apiKey!.value = '';
    typeKey();
    assert.equal(
      Object.hasOwn(editor.data(), 'apiKey'),
      false,
      'Clearing a replacement does not revive removal',
    );
    ...
    assert.equal(
      Object.hasOwn(editor.data(), 'apiKey'),
      false,
      'New endpoints can omit an empty key',
    );
    ...
    assert.equal(
      Object.hasOwn(editor.data(), 'apiKey'),
      false,
      'Loading accepted state clears acknowledged removal',
    );
  } finally {
    dispose();
  }
