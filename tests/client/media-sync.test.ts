...
test('ordered media snapshots preserve early progress and reject stale history after reconnect', async () => {
  ...
  try {
    ...
    await initial;
    assert.equal(
      state.mediaJobs[1]?.progress?.value,
      2,
      'History cannot overwrite early WS progress',
    );
    ...
    assert.equal(
      state.mediaJobs[3],
      undefined,
      'Deletion rejects late history and action responses',
    );
  } finally {
