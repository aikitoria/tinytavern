...
test('assigned IDs reach in-flight edits and every dependent generation draft reference', () => {
  ...
  submitted.mediaRendering.folders = [{ id: 'folder-draft', name: 'Folder' }];
  submitted.mediaRendering.shortcuts = [{ id: 'shortcut-draft', name: 'Shortcut', workflowId: workflow.id }];
  submitted.mediaFavorites = [{ id: 'favorite-draft', name: 'Favorite', workflowId: workflow.id, presetId: '3' }];
  ...
  const adopted = reconcileMediaDraft(current, submitted, saved, {
    media_workflows: { 'draft-1': '21' },
    media_workflow_folders: { 'folder-draft': '22' },
    media_shortcuts: { 'shortcut-draft': '23' },
    media_favorites: { 'favorite-draft': '24' },
    avatar_prompts: { 'avatar-draft': '25' },
  });
  ...
  assert.equal(adopted.imageGeneration.promptPresets!.avatar!.presets[0]!.prompt, 'Edited while saving');
...
test('entity saves use the draft revision and fetch intervening settings changes', async () => {
  ...
  mock.module('../../client/src/state/api.ts', () => ({
    api: {
      async mediaEntity(_table: string, _method: string, _id: string, body: Record<string, unknown>) {
        sent = body;
        return {
  ...
  assert.equal(sent!.expectedRevision, 2, 'A newer store row cannot authorize overwriting the older draft');
  assert.equal(refreshes, 1, 'A response that skips a settings revision requires fetching intervening changes');
...
test('same-name shortcuts keep distinct assigned IDs across edits, deletion and insertion during save', () => {
  ...
  let result = reconcileMediaDraft(current, submitted, saved, { media_shortcuts: { '1': '2', '2': '3' } });
  ...
  result = reconcileMediaDraft(current, submitted, saved, { media_shortcuts: { '1': '2', '2': '3' } });
