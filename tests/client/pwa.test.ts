...
test('dev and prod PWAs have disjoint Android scopes even when ports are ignored', () => {
  ...
  const scopes = manifests.map(
    (manifest, index) => new URL(manifest.scope, origins[index]).pathname,
  );
  for (const [index, manifest] of manifests.entries()) {
    const launch = new URL(manifest.start_url, origins[index]);
    assert(launch.pathname.startsWith(scopes[index]!));
    assert(
      !launch.pathname.startsWith(scopes[1 - index]!),
      'Android must not resolve the other installed app',
    );
  }
