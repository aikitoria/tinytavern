...
const { describeImage, descriptionWorkflow } = await import('../server/src/mediaDescription.ts');
...
const comfy = createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === '/upload/image') {
      ...
      socket.send(JSON.stringify({ type: 'execution_start', data: { prompt_id: body.prompt_id } }));
      socket.send(
        JSON.stringify({
          type: 'executing',
...
try {
  initMediaWorker();
  const progress: ImageDescriptionProgress[] = [];
  failDelete = true;
  ...
  assert.equal(result, '  A detailed uploaded image description.\nSecond line.  ');
  assert(
    progress.some(
      (value) =>
        value.progress.value === 16 &&
        value.progress.max === 512 &&
        value.progress.node?.name === 'Generate Text',
    ),
  ...
  assert.equal(stmt('SELECT count(*) AS count FROM media_assets').get()!.count, 1);
  await until(() => uploads.size === 0);
  assert.equal(removed.length, 1, 'Failed remote deletion is retried after temporary job removal');
  ...
  console.log(
    'Comfy image descriptions: unchanged workflow, token progress, text SSE, explicit-save boundary, cancellation and durable upload cleanup passed',
  );
} finally {
