...
import { videoPreviewFrame } from './helpers/videoPreview.ts';
...
const server = createServer(async (request, response) => {
  ...
  if (path === '/prompt') {
    ...
    socket?.send(
      JSON.stringify({ type: 'progress', data: { prompt_id: body.prompt_id, value: 1, max: 20 } }),
    );
    ...
    socket?.send(
      JSON.stringify({ type: 'progress', data: { prompt_id: body.prompt_id, value: 2, max: 20 } }),
    );
    socket?.send(
      JSON.stringify({
        type: 'progress_state',
...
try {
  ...
  for (const mode of ['render', 'prepare-and-render', 'prepare-then-render', 'video']) {
    ...
    console.log(
      `${mode}: first preview in ${Math.round(previews.get(job.id)!.time - began)} ms, including the delayed handshake`,
    );
  }
} finally {
