...
async function launch() {
  const child = fork(join(import.meta.dirname, 'helpers/media-restart-worker.ts'), {
...
try {
  ...
  console.log(
    'Real process crashes recover acceptance, queue/running state, partial AV1 downloads and cleanup without rerendering',
  );
} finally {
