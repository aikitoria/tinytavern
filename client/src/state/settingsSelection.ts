...
export type SettingsEntityKey = 'activeEndpointId' | 'defaultPresetId' | 'defaultPersonaId' | 'defaultTemplateId';
...
const selectionPatch = (key: SelectionKey, id: SelectionId): Partial<Settings> =>
  mediaPromptKey(key) ? { [key]: { defaultPresetId: id } } : { [key]: id };
    ...
    for (;;) {
      ...
      try {
        if (selected(key) !== target) {
          try {
            applySettings(await api.putSettings(selectionPatch(key, target), state.settings.revision));
          } catch (err) {
...
export const selectSettingsEntity = (key: SettingsEntityKey, id: number | null): Promise<void> => select(key, id);

export const selectMediaPromptPreset = (key: MediaPromptKey, id: string | null): Promise<void> => select(key, id);
