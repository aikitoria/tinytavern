...
- Prettier enforces the repository-wide 120-character wrapping target. Run the container formatter after edits.
...
- Schema lives in `server/src/db/schema.ts`. Baseline version is 81; current version is 88. Add future upgrades through `migrate()` in `db.ts` and update the fresh schema, including persisted JSON, file references and ownership. Never replay seeds on existing data.
...
- Media libraries use SQLite rows and server-assigned AUTOINCREMENT IDs. Settings JSON stores scalar preferences only. Normal refreshes use versioned collection snapshots; aggregate DTOs remain at explicit compatibility and import/export boundaries. Deleted workflows and prompts retain their identities for history. Render details use captured names and parameter values.
...
- Review drafts create their prompt discussion from the outset; saved prompts seed a discussion at creation, and upgrades convert old drafts once. Unattended jobs keep their lightweight preparation path. Media drafts may own a real conversation through `media_drafts.conversation_id`. Its captured `conversations.prompt_context_json` bypasses chat templates; messages retain full branch/swipe semantics. Media conversations exist only in the media panel, never in chat navigation/search or standalone conversation copies/transfers. Render jobs capture a chosen reply via `prompt_message_id` plus immutable prompt text. Deleting the last draft job releases its discussion with the deep-tree-safe deletion helper.
...
- Workflows declare typed image/video inputs, prompt bindings and exposed controls. Media workflows have exactly one output node; returned assets determine image/video handling, and videos are original AV1 WebM. Text outputs use an explicit node binding and their own validation.
- Media/prompt bindings live in `shared/src/media.ts`; exposed controls live in `shared/src/workflowInputs.ts`. Use a Comfy primitive Text node for `{{prompt}}` to preserve braces during export. Annotated `_meta.title` declares `[image:input1]` through `[image:input64]`, `[video:input1]` through `[video:input64]` (native Load Video `file` field), `[prompt]`, and exposed `[input]` controls. Preserve graph wiring; jobs and recipes reference saved workflows by ID and reruns use the current graph.
- Workflow settings include folders, chat and standalone prompt presets, and automatic media sources. Generation settings hold the ComfyUI connection, defaults, shortcuts and toolbar favorites. Each favorite pairs a chat prompt preset with a workflow that has no media inputs.
- Media prompt macros are `{{input1_prompt}}` through `{{input64_prompt}}`, matching the image binding number directly, including sparse inputs. Resolve against captured inputs by slot; unavailable input prompts expand to empty text. Human names before the annotation are display labels only. Workflows use strict JSON, numeric seeds and numbered media bindings. Legacy filename inference, arbitrary binding names and seed macros are unsupported.
- Capture prompts and input associations per job/recipe, but never copy workflow graphs into them. Chat preparation, including images from references, preserves the complete chat prefix; gallery preparation uses standalone prompts. Selecting a variation changes only the preview, never the working editor. Save attaches results atomically; closing started drafts leaves work running.
...
- Deleting source media releases inactive reference pins and preserves a missing input slot; active jobs may keep inputs until completion. Conversation JSON is version 1, including raster recipes/references but omitting video files. Validate transfers fully before writes and remove files on rollback.
