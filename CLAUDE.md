...
- Schema lives in `server/src/db/schema.ts`. Baseline version is 81; current version is 83. Add future upgrades through `migrate()` in `db.ts` and update the fresh schema, including persisted JSON, file references and ownership. Never replay seeds on existing data.
