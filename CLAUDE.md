...
**Page URLs** (`state/pageLocation.ts`): hash routes retain the background chat, gallery item and filters, settings section/entity, chat/map/trace view, and media operation/job/context with its return URL. Examples: `#70/gallery/123`, `#70/settings/templates/5`, `#70/media/video/<job-id>`. Reload restores navigation after initial data loading; jobs and progress are fetched from the server. Browser Back/Forward goes through the mounted settings/conversation dirty guard or media draft save/discard path. App entries carry `tinytavernPageIndex` in history state; guarded traversal returns to the original entry before asking, then repeats the traversal on approval. Cancel must not rewrite or consume a history entry. Hardware mouse Back first invokes the top visible UI surface’s existing Back/Close action (`state/uiBack.ts`), including dropdowns, image viewers and the mobile entity editor; it consumes the whole pointer/mouse/auxclick sequence so a single press cannot close two layers. With no dismissible UI, browser navigation is left untouched. URL updates preserve chat selection and never serialize prompts, credentials, signed media URLs, or generation buffers.
...
Downloads stream into exclusive `.part` files, bounded to 64 MB raster or 1 GB video. Validate raster bytes and dimensions; video inspection verifies the WebM EBML type and AV1 codec through FFprobe. Sync and rename before recording the result; preserve original WebM bytes. Caddy serves signed `/images/` rasters, WebM and thumbnails with range support. Node's direct media serving also supports ranges and streams with backpressure. FFmpeg is installed in both root Dockerfile stages used by Compose.
...
`GalleryModal`/`GalleryGrid` retain O(n) justified layout, binary-search row virtualization and focus/scroll anchors. Gallery picker mode filters to images and keeps ordered slot selection separate from bulk deletion. Tools and Jobs belong in the grid header and are hidden in details. `MediaToolsModal` is fullscreen, with a non-scrolling preview stage and compact variation/actions footer. The originating gallery stays mounted but inactive under media tools/pickers. Composer has Create image from chat and Create video from chat; Edit image opens standalone, including image actions from chat. Image actions prefill Reference 1, first frame, or video reference. Message/code-block menus share `MediaPromptMenuItems` and copy exact text into Final prompt with chat as the destination. Global jobs and open variations refresh on reconnect without overwriting unsaved edits.
