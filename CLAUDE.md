...
- Schema lives in `server/src/db/schema.ts`. Baseline version is 73; current version is 77. Add future upgrades through `migrate()` in `db.ts` and update the fresh schema, including persisted JSON, file references and ownership. Never replay seeds on existing data.
...
- Workflows declare named image inputs, prompt bindings and exposed controls. Media workflows have exactly one output node; returned assets determine image/video handling, and videos are original AV1 WebM. Text outputs use an explicit node binding and their own validation.
- Image/prompt bindings live in `shared/src/media.ts`; exposed controls live in `shared/src/workflowInputs.ts`. Use a Comfy primitive Text node for `{{prompt}}` to preserve braces during export. Legacy loader filenames `source.png`, `first_frame.png`, and `referenceN.png` are converted on import. Annotated `_meta.title` declares `[image:input1]` through `[image:input64]`, `[prompt]`, and exposed `[input]` controls. Preserve graph wiring and captured workflow snapshots.
- Workflow settings include folders, chat and standalone prompt presets, and automatic image sources. Generation settings hold the ComfyUI connection, defaults, shortcuts and toolbar favorites. Each favorite pairs a chat prompt preset with a workflow that has no image inputs.
- Media prompt macros are `{{input1_prompt}}` through `{{input64_prompt}}`, matching the image binding number directly, including sparse inputs. Resolve against captured inputs by slot; unavailable input prompts expand to empty text. Human names before the annotation are display labels only. Convert legacy named bindings at import and migration boundaries, preserving associations.
