...
**Server-authoritative state, clients are pure viewers.** All state lives in SQLite (`server/src/db.ts`, schema migrations via `PRAGMA user_version`, DELETE rollback journaling with `synchronous=EXTRA`). All SQL goes through `stmt()` from db.ts — a memoized prepared-statement cache; never call `db.prepare` directly. Clients never mutate locally; they call REST endpoints under `/api/` and receive updates over the `/ws` WebSocket:
...
**Settings workspace** (`SettingsModal.tsx`): a full-screen page using the same `Modal fullscreen` shell as Gallery (`styles/pages.css`). Desktop section navigation sits beside the editor; below 1100px the shared Select in the header replaces it. Fields are centered at a maximum 960px width, while save bars span the available workspace. Forms use shared controls and flat sections; there are no settings-only input sizes or card surfaces. On mobile the whole page scrolls, including the header, while entity editors retain their list/detail navigation. Back, Escape, section changes, and entity changes go through the existing Save/Discard/Cancel guard. Each editable setting uses `SettingLabel` and a conditional revert arrow. `createDefaultField` in `SettingField.tsx` tracks both native edits and imperative `.value`/`.checked` loads; Select resets call its explicit `change` method so dependent editors update too. Defaults are the built-in new-entity values, never an editable entity named or selected as default. Reverts affect one draft field and retain the existing save/guard behavior; avatar removal remains immediate. Section headings use spacing rather than divider lines, with a smaller gap below than above.
...
**Image generation** (`client/src/images/`): a built-in feature shared by chat, gallery, and avatars. `imageGeneration.tsx` exports composer tools and slash commands, image-message rendering and swipe behavior, and the Settings → Images editor, a single page with all prompt and rendering sections visible. `MessageNode` and `messageSwipe.ts` call the image behavior directly; there is no plugin registry. `/image [instruction]`, `/imagechar [instruction]`, and `/imageface [instruction]` expand `{{instruction}}` client-side, then run a **tool generation**; swiping forward while its prompt is streaming cancels it before rendering. `Settings.imageGeneration` stores prompt presets, workflows, active selections, and the ComfyUI URL with the usual revision guard and settings invalidation. Migration 27 preserves the former `pluginSettings.imageGeneration` object verbatim, removes the old container, and advances the settings revision. The client still normalizes older single-prompt and single-workflow customizations. The Chat image revision section in Settings → Images owns the global `Settings.imageGeneration.promptRevisionTemplate` for chat image edits; it has no character overrides. `{{instruction}}` and optional `{{prompt}}` expand in one pass, preserving literal macro text in user input. Settings → Gallery owns the separate `Settings.gallery.promptRevision` standalone template: `systemPrompt`, `userMessage`, `reasoningPrefill`, and `messagePrefill`. `buildGalleryRevisionPrompt` builds exactly those messages without chat context or an implicit outer prompt; `streamChatCompletion` applies the global endpoint’s sampling settings and prefill protocol, emits the visible seed once, and keeps reasoning hidden. Migration 28 replaces the old gallery default with the standalone default; custom revision text is retained verbatim in the new user message, after an explicit original-prompt block.

**Tool generations** (`POST /api/conversations/:id/tool`): an image prompt runs as a foreground generation streaming into a `role: 'tool'` message appended at the active leaf — so the normal streaming/retry machinery applies unchanged. The prompt gets the full chat context plus the macro-expanded prompt as a trailing user turn (`buildToolPrompt`; `{{char}}`/`{{user}}` expand server-side, no name prefill), snapshotted at route time via the generation's `promptOverride` so retries stay consistent. Tool messages are chat-visible but skipped by `buildChatMessages`, so they never enter later prompt history; they can't be swiped/advanced/resumed (assistant-role guards on both sides). Multiple tool prompts may stream concurrently because each is an independent snapshot; deleting an older tool block preserves active descendants and only stops streams whose rows are actually deleted. Starting one discards an in-flight speculative swipe without refilling (a branch switch restarts speculation anyway).
...
**Saved image gallery** (`server/src/routes/gallery.ts`): saving a specific message image swipe copies the raster to a gallery-owned file and snapshots its prompt, render configuration, character name, and optional source links in `gallery_items`. The source and character foreign keys use `ON DELETE SET NULL`; they are navigation/filter metadata, never ownership. `POST /api/gallery/upload` accepts one raw PNG/JPEG/WebP original per request (64 MB limit), validates its bytes with `rasterImageFormat`, and assigns an optional character or the Uploads group. Uploaded files have no source links or render configuration. Gallery rows persist `image_width`/`image_height`; `imageDimensions.ts` reads headers without decoding the raster, including JPEG EXIF axis rotation. Migration 26 backfills existing files using bounded reads. Originals are retained for display and download; there are no generated thumbnails. Deletion owns only gallery paths, and `sweepOrphanedImages()` treats both message and gallery image paths as live references.

**Gallery browser** (`client/src/components/GalleryModal.tsx`, `GalleryGrid.tsx`, `GalleryDetail.tsx`, `galleryModel.ts`): prompt search, character filters with counts, sorting, and image-size preferences feed O(n) justified rows. The grid virtualizes rows with binary-search culling and rAF scroll updates, retains the focused tile, and preserves its scroll anchor across layout changes and detail navigation. Selection operates on filtered results and bulk-deletes through one transactional route. Multi-file upload/drop uses two concurrent raw requests. The detail view navigates the filtered collection, keeps per-image prompt drafts, downloads originals, and exposes source links plus an optional panel for LLM prompt revision and generation. Revisions use the template from Settings → Gallery and abort/restore the draft on navigation. Rendering uses the submitted prompt with the current workflow or saved fallback to create a separate gallery item. Job-scoped SSE progress/previews update a compact activity strip independently of grid layout; pending jobs live in the shared store and finish after the gallery closes.

**Avatar generation** (`server/src/routes/avatarGenerate.ts`): an interactive, client-driven flow — nothing is stored until the user confirms. `POST /api/{characters,personas}/:id/avatar/prompt` takes the active avatar preset's paired `{ prompt, context }` templates, expands `{{name}}`/`{{char}}`/`{{user}}`/`{{description}}`/`{{personality}}`/`{{scenario}}`/`{{firstMessage}}` from the entity row (characters have no `description` column — `{{description}}` maps to `personality`, matching card import/export), sends them as separate system/user messages, and SSE-streams the completion (`data: {d}` deltas, `{error}`, `{done}`) via `streamChatCompletion` in generation.ts (streaming sibling of `chatCompletionOnce`, same refusal/empty diagnosis, global active endpoint); a per-entity in-memory guard 409s a second concurrent stream. Old clients that omit `context` receive the server's field-serialized fallback. `POST /api/avatar/render` is stateless: `{ prompt, image: { workflow, comfyUrl } }` (validated by `parseImageConfig`) → image bytes via comfy.ts's `renderToBuffer`; a caller-provided `jobId` gets sampler progress and transient TAESD preview data URLs through a private job-scoped SSE stream registered before submission, preventing first-step races and cross-modal leakage. Saving is the normal `PUT .../avatar` route, so PNG enforcement lives there. On the client the built-in image module owns the flow: the paired avatar prompt preset + a dedicated `avatarWorkflow` selection ('' falls back to the /image workflow) in Image Generation settings, and `AvatarGenerateModal` (`client/src/images/AvatarGenerateModal.tsx`) opened from the `generate` prop on `AvatarRow` — it streams the prompt into an editable textarea, auto-renders with the live preview in the modal, and offers regenerate (fresh seed), edit-and-rerender, save, or cancel. The mock records completion requests (streaming or not) at `/control/last-completion` for e2e macro assertions.
...
**Steered regeneration** (`POST /api/messages/:id/regenerate`): the prompt is snapshotted at route time via `promptOverride`, and the one-off revision request never enters message history. Assistant results remain assistant sibling swipes: the original reply is included and followed by an alternating-safe user turn rendered through the conversation's resolved `steerTemplate` (`resolveSteerTemplate`: character inline `customTemplate` → character `templateId` → global default template → `DEFAULT_STEER_TEMPLATE`; `{{instruction}}` slot). Image tool results are appended as child tool messages after the source (image alternatives remain the special within-message image swipes), retain their render config, and automatically render a fresh image. `buildSteeredToolPrompt` preserves the unchanged roleplay history as the upstream prefix (for prefix caching and contextual references), then appends the original image prompt as an assistant turn and the global Images revision instruction as a user turn. The default `[IMAGE PROMPT REVISION TASK]` marks history as reference-only and separately tags `<original_image_prompt>` and `<revision_instruction>` so the model does not continue the roleplay.
