...
- Schema lives in `server/src/db/schema.ts`. Baseline and current version are 73. Add future upgrades through `migrate()` in `db.ts` and update the fresh schema, including persisted JSON, file references and ownership. Never replay seeds on existing data.
...
- Image/prompt bindings live in `shared/src/media.ts`; exposed controls live in `shared/src/workflowInputs.ts`. Use a Comfy primitive Text node for `{{prompt}}` to preserve braces during export. Reserved loader filenames are `source.png`, `first_frame.png`, and `referenceN.png`. Annotated `_meta.title` declares `[image:name]`, `[prompt]`, and exposed `[input]` controls. Preserve graph wiring and captured workflow snapshots.
...
- Media prompt macros are `{{input1_prompt}}` through `{{input64_prompt}}`, following `mediaInputSlots()` order. Resolve against captured inputs by slot, preserving empty positions; unavailable input prompts expand to empty text. Image input names are graph bindings, not prompt macro names.
