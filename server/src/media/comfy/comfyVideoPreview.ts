...
export function parseVideoPreviewFrame(frame: Buffer) {
  ...
  if (frame.readUInt32BE(0) !== 1 || frame.readUInt32BE(4) !== 1 || frame.readUInt32BE(8) !== 1) return null;
...
export class ComfyVideoPreview {
  ...
  static fromEvent(data: unknown, executingNode: string | null): ComfyVideoPreview | null {
    ...
    if (typeof length !== 'number' || !Number.isInteger(length) || length < 1 || length > MAX_FRAMES) return null;
  ...
  accept(frame: Buffer): MediaVideoPreview | null {
    ...
    if (!decoded || decoded.nodeId !== this.metadata.nodeId.slice(0, 15) || decoded.index >= this.metadata.frameCount)
      return null;
