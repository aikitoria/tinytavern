...
function parseInputs(
  ...
  return value.flatMap((raw) => {
    ...
    const asset = stmt(`
      ...
      WHERE a.id = ? AND a.kind IN ('image', 'video') AND a.reference_deleted = 0
        AND EXISTS (SELECT 1 FROM media_owners o WHERE o.asset_id = a.id)
    `).get(input.assetId as number);
    if (!asset) {
      if (allowMissingInherited) return [];
      throw new HttpError(409, 'A selected reference media asset is no longer available');
    }
    return [
      {
...
function validateJobWorkflow(workflow: MediaWorkflow) {
...
function draftConfiguration(
  ...
  try {
    ...
    const compiled = validateJobWorkflow(resolved.workflow);
    ...
    return JSON.stringify({
      ...
      seedOverride: seedOverride === undefined ? resolved.configuration.seedOverride : seedOverride,
    });
  } catch (err) {
  ...
  try {
    transaction(() => {
      ...
      INSERT INTO media_jobs (
        workflow_id, chat_preset_id, standalone_preset_id, instruction, prompt,
    ...
    `).run(
          workflowId,
          conversationId == null ? null : presetId,
          conversationId == null ? presetId : null,
          instruction,
          prompt,
          conversationId,
          destination,
...
function resolvePresetReference(
  ...
  const id = requested === undefined ? inherited : requested;
  if (id === null) {
    return null;
  }
  const table = chat ? 'media_chat_prompts' : 'media_standalone_prompts';
  if (stmt(`SELECT id FROM ${table} WHERE id = ? AND deleted_at IS NULL`).get(id)) {
    return id;
  }
  // An unchanged historical selection must not block editing or rerunning its saved prompt.
  if (id === inherited) {
    return null;
  }
...
export function startMediaJob(row: MediaJobRow, body: JobBody, prepare: boolean) {
  ...
  const { configuration, workflow } = resolved;
  const compiled = validateJobWorkflow(workflow);
  ...
  if (
    ...
    throw new HttpError(400, `Select the required media: ${requiredSlots.join(', ')}`);
  }
  for (const input of compiled.mediaInputs) {
  ...
  configuration.workflowName = workflow.name;
  configuration.workflowParameters = compiled.controls.map((control) => ({
    label: control.label,
    value: configuration.workflowValues?.[control.key] ?? control.value,
  }));
  configuration.characterIds = captureMediaCharacters(row, configuration);
  const context = prepare ? prepareContext(row, configuration, workflow) : {};
  const now = Date.now();
