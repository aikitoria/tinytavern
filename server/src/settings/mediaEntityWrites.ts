...
import { MEDIA_ENTITIES, mediaEntityDto, encodeMediaField, type MediaEntityTable } from '../settings/mediaEntities.ts';
import { touchMediaSettings } from '../settings/settingsStore.ts';
import { parseMediaWorkflow, supportsMediaFavorite, validateWorkflowSelection } from '../media/mediaSettings.ts';
import { requireMediaWorkflow } from '../media/mediaWorkflows.ts';
import { type EntityConfig, entityFields } from '../routes/shared/entityRoutes.ts';

/** Validate only references to this workflow, without reading or parsing the library. */
export function validateWorkflowUpdate(workflow: MediaWorkflow): void {
  ...
  const favorite = stmt('SELECT id FROM media_favorites WHERE workflow_id = ? LIMIT 1').get(workflow.id);
...
/** Batch saves run this after retargeting existing shortcuts and favorites. */
export function deleteMediaDependentRows(table: MediaEntityTable, id: number): void {
...
function cleanupReferences(table: MediaEntityTable, id: number, deferDependentDeletes: boolean): void {
  if (!deferDependentDeletes) {
  ...
  for (const [column, target] of prompts) {
    if (table === target) {
      stmt(`UPDATE media_workflows SET ${column} = NULL, revision = revision + 1 WHERE ${column} = ?`).run(id);
    }
  }
}

export function mediaEntityConfig(
  table: MediaEntityTable,
  options: { deferWorkflowValidation?: boolean; deferDependentDeletes?: boolean } = {},
): EntityConfig<{ id: number; name: string; revision: number; folderId: string | null }> {
  ...
  return {
    ...
    toDto: (row) => ({
      ...
      settingsRevision: Number(
        stmt("SELECT json_extract(value, '$.revision') AS revision FROM settings WHERE key = 'app'").get()?.revision ??
          0,
      ),
    }),
    guard: (body, current) => {
    ...
    prepare: (body, current) => {
      ...
      if (table === 'media_workflows') {
        ...
        if (current && !options.deferWorkflowValidation) {
        ...
        for (const [key, target] of [
          ...
          if (ref != null && !stmt(`SELECT id FROM ${target} WHERE id = ? AND deleted_at IS NULL`).get(ref))
      ...
      if (table === 'media_shortcuts' || table === 'media_favorites') {
        ...
        for (const [field, target] of references) {
          ...
          if (typeof id !== 'string' || !stmt(`SELECT id FROM ${target} WHERE id = ? AND deleted_at IS NULL`).get(id)) {
        ...
        if (table === 'media_shortcuts') {
          ...
          throw new HttpError(400, 'Favorites require a workflow with a prompt and no media inputs');
        }
      }
      if (spec.folder) {
      ...
      if ((table === 'media_chat_prompts' || table === 'media_standalone_prompts') && !String(instruction).trim()) {
    ...
    prepareDelete: (id) => cleanupReferences(table, id, options.deferDependentDeletes ?? false),
    afterWrite: touchMediaSettings,
  };
}

export function mediaFolderConfig(
  table: MediaEntityTable,
): EntityConfig<{ id: number; name: string; createdAt: number }> {
  const spec = MEDIA_ENTITIES[table];
  if (!spec.folder) {
    throw new Error('This entity has no folders');
  }
  return {
    table: spec.folder,
    invalidateEntity: 'settings',
    ...
    prepare: (body, current) => {
      const name = body.name === undefined && current ? current.name : requiredString(body, 'name');
      if (stmt(`SELECT id FROM ${spec.folder} WHERE name = ? COLLATE NOCASE AND id <> ?`).get(name, current?.id ?? 0)) {
        throw new HttpError(409, 'A folder with this name already exists');
      }
      return { ...body, name };
    },
    prepareDelete: (id) => {
      stmt(`UPDATE ${table} SET folder_id = NULL, revision = revision + 1 WHERE folder_id = ?`).run(id);
    },
